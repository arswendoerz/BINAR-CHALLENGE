...
exports.createCar = async (req, res, next) => {
    try {
        const data = await carService.createCar(req.body, req.file); // Ambil file dari req.file
        successResponse(res, data);
    } catch (error) {
        next(error);
    }
...
exports.updateCar = async (req, res, next) => {
    const { id } = req.params;
    try {
        const data = await carService.updateCar(id, req.body, req.file); // Ambil file dari req.file
        successResponse(res, data);
    } catch (error) {
        next(error); 
    }
